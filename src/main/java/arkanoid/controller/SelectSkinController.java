...
public class SelectSkinController {
    ...
    private void showBallSkins() {
        ...
        for (int i = 1; i <= 3; i++) {
            ImageView img = new ImageView(new Image(getClass().getResourceAsStream("/assets/ball" + i + ".png")));
            img.setFitHeight(80);
            img.setFitWidth(80);
            int index = i;
    ...
    private void showPaddleSkins() {
        ...
        for (int i = 1; i <= 3; i++) {
            ImageView img = new ImageView(new Image(getClass().getResourceAsStream("/assets/paddle" + i + ".png")));
            img.setFitHeight(80);
            img.setFitWidth(160);
            int index = i;
            img.setOnMouseClicked(e -> {
    ...
    private void goBack(ActionEvent event) {
        try {
            ...
            javafx.fxml.FXMLLoader loader =
                    new javafx.fxml.FXMLLoader(getClass().getResource("/ui/fxml/Main.fxml")); // dùng đường dẫn tuyệt đối
            javafx.scene.Parent root = loader.load();

            // Giữ style giống các màn khác
            javafx.scene.Scene scene = new javafx.scene.Scene(root, 800, 600);
            scene.getStylesheets().add(
                    getClass().getResource("/ui/css/style.css").toExternalForm()
            );
